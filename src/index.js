...
export default class AnimateList extends Component{
    ...
    render(){
        ...
        return(<ScrollView style={{height:300,width:200,backgroundColor:'white'}}  
        onScroll={(e)=>{
    ...
    renderList(data,index){
        return(<AnimateItem onLayout={(e)=>{this.heightList[index]=e.nativeEvent.layout.height;}} width={200} style={{height:50,width:200}} key={index}
            ref={(ref)=>{this.itemRefs[index]=ref}}>
        {data}
        </AnimateItem>);
    }
    handleScroll(y){
        let i=this.getItemIndex(y+300);
        if(i<0)return;
...
export class AnimateItem extends Component{
    ...
    startAnimate(){
        ...
        if(!this.isAnimate){
            ...
            this.isAnimate=true;
            this.state.offset.setValue(this.props.width);
